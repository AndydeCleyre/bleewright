...
tmpfile=$(mktemp)
targetfile=~/.config/rio/themes/blee.toml
cp "$targetfile" "$tmpfile"

lineinfile add --backrefs --after-first '\[colors\]' \
  --regexp '^(background = ).*' "\1'$bg'" \
  "$tmpfile"

lineinfile add --backrefs --after-first '\[colors\]' \
  --regexp '^(cursor = ).*' "\1'$cursor'" \
  "$tmpfile"

lineinfile add --backrefs --after-first '\[colors\]' \
  --regexp '^(white = ).*' "\1'$fakewhite'" \
  "$tmpfile"

lineinfile add --backrefs --after-first '\[colors\]' \
  --regexp '^(light_white = ).*' "\1'$fakewhitesat'" \
  "$tmpfile"
